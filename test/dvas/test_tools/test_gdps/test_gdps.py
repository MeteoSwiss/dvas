...
def gdp_2_prfs_real(db_init):
    ...
    data_1 = pd.DataFrame({'alt': [16168.318359375, 16172.6962890625,   16177.47265625,
                           ...
                           'flg': [0] * 32, 'tdt': 1.e9*np.arange(0,32,1),
...
def test_combine(gdp_1_prfs, gdp_2_prfs_real, gdp_3_prfs):
    ...
    out_a = combine(gdp_2_prfs_real, binning=3, method='delta', n_cpus=16, mask_flgs='incomp',
                    chunk_size=3)
    out_b = combine(gdp_2_prfs_real, binning=3, method='delta', n_cpus=16, mask_flgs='incomp',
                    chunk_size=4)
