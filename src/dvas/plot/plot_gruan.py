...
def gdps_vs_cws(gdp_prfs, cws_prf, ref_var='alt', tag=None, save_loc=None):
    """ Makes a plot comparing different GDPs and their associated Combined working measurement
    standard.
    ...
    Args:
        ...
        ref_var (str, optional): reference variables for the plots, either 'tdt' or 'alt'. Defaults
            to 'alt'.
    ...
    # Start the plotting
    plt.close(55)
    plt.figure(55, figsize=(pu.WIDTH_TWOCOL, 5.0))
    ...
    # TODO: add checks that all profiles are on the same levels.
    # What is my reference values.
    x = cws.index.get_level_values('alt')

    # Very well, let us plot all these things.

    for (gdp_ind, gdp) in enumerate(gdps):
