...
def basic_plot(items, fig_num=None, save_fn=None):
    ...
    for arg in items:
        ...
        x_data_s = arg.index.astype('timedelta64[s]')

        xmin = np.nanmax([xmin, np.min(x_data_s)])
        xmax = np.nanmin([xmax, np.max(x_data_s)])

        ax1.plot(x_data_s, arg.values, linestyle='-', drawstyle='steps-mid')
...
def basic_alt_plot(items, fig_num=None, save_fn=None):
    """ Create a basic plot.
    ...
    for arg in items:
        ...
        xmin = np.nanmax([xmin, np.nanmin(arg.values)])
        xmax = np.nanmin([xmax, np.nanmax(arg.values)])

        ax1.plot(arg.values, x_data_s, linestyle='-', drawstyle='steps-mid')

    # Deal with the axes
    ax1.set_ylabel(r'Alt [m]')
    # TODO: Also add the x label
