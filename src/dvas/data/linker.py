...
class FileHandler(AbstractHandler):
    ...
    @abstractmethod
    def metadata_fct(self, *args, **kwargs):
        """"""
            ...
            return ''.join(
                [arg for arg in chain(
                    *zip_longest(
                        ...
                        [
                            self.metadata_fct(item) for item in
...
class CSVHandler(FileHandler):
    ...
    FILE_SUFFIX = re.compile(r'\.csv', re.IGNORECASE)
    FILE_INSTR_TYPE_PAT = re.compile(
...
class GDPHandler(FileHandler):
    ...
    FILE_SUFFIX = re.compile(r'\.nc', re.IGNORECASE)
    FILE_INSTR_TYPE_PAT = re.compile(
...
#             origdata_cfg_prm = self._origdata_config_mngr.get_all_default(
#                 [instr_type_name, prm_abbr]
#             )
#
#             # Get raw data config param
...
#
#             # Get config
#             gdpdata_cfg_prm = self._gdpdata_config_mngr.get_all_default(
#                 [instr_type_name, prm_abbr]
#             )
#
#             #try:
